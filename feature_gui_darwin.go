...
func enableGui(project Project) (map[docker.Port][]docker.PortBinding, []string, error) {
    ...
    if displayVariable == "" {
        return portBindings, envVariables, errors.New("DISPLAY variable is empty. (Did you install XQuartz properly?)")
    } else {
        ip, err := bridgeUnixSocketToPort(displayVariable, guiPortNumber)
        if err != nil {
            return portBindings, envVariables, errors.New("Network bridging failed: " + err.Error())
        } else {
            // log.Debug("bridged to ip:", string(ip))
            envVariables = append(envVariables, "DISPLAY=" + ip + ":0")
            return portBindings, envVariables, nil
        }
    }
}
